<script lang="ts">
	...
	import { extractZip, getDateFromFileName, getParticipantFromFileName, objectIsEmpty } from '$lib/postprocessing/utils';
	...
	function handleSubmit() {
		...
		extractZip(files).then((data) => {
			let result = data.map((file: { name: string, data: any[] }) => {
				...
				return {participant: participant, date: date, info: info}
			})
